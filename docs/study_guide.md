...
### Manager (Jack)
- **The accuracy gate is pure rules, not the LLM.** retune-vs-proceed is decided by comparing accuracy to the target (0.516). An LLM (Llama-3.1) only writes the human-readable rationale. Justification: the decision that controls the whole loop must be deterministic and auditable.
...
- **Honest baseline = 0.516 (all-neutral).** Any model must beat this to matter. The target was originally **0.60, set before anyone measured** whether the task supports it; it has since been lowered to **0.516 — the all-neutral baseline itself**, so "clearing the gate" now means "no worse than always guessing neutral."
