...
*Awaiting a fresh run. The previous run also reweighted the loss toward the
evaluator's flagged class, which changed two things at once, so its numbers are
not a clean test of "does training in the loop help?". That reweighting has been
removed and the experiment is being re-run with training-in-the-loop as the only
change.*
