...
None of those output files are included in the submission — this folder ships the
code and the write-up only, since the experiment is a side investigation rather
than part of what we built for the project. The numbers reported below come from
the run described there, and re-running the script reproduces them.
